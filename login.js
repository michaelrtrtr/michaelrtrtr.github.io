function discordIcon() {
  return `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.317 4.492c-1.53-.69-3.17-1.2-4.885-1.49a.075.075 0 0 0-.079.036c-.21.369-.444.85-.608 1.23a18.566 18.566 0 0 0-5.487 0 12.36 12.36 0 0 0-.617-1.23A.077.077 0 0 0 8.562 3c-1.714.29-3.354.8-4.885 1.491a.07.07 0 0 0-.032.027C.533 9.093-.32 13.555.099 17.961a.08.08 0 0 0 .031.055 20.03 20.03 0 0 0 5.993 2.98.078.078 0 0 0 .084-.026 13.83 13.83 0 0 0 1.226-1.963.074.074 0 0 0-.041-.104 13.201 13.201 0 0 1-1.872-.878.075.075 0 0 1-.008-.125c.126-.093.252-.19.372-.287a.075.075 0 0 1 .078-.01c3.927 1.764 8.18 1.764 12.061 0a.075.075 0 0 1 .079.009c.12.098.246.195.373.288a.075.075 0 0 1-.006.125c-.598.344-1.22.635-1.873.877a.075.075 0 0 0-.04.105c.36.687.772 1.341 1.225 1.962a.077.077 0 0 0 .084.028 19.963 19.963 0 0 0 6.002-2.981.076.076 0 0 0 .032-.054c.5-5.094-.838-9.52-3.549-13.442a.06.06 0 0 0-.031-.028Z"/></svg>`;
}

function startDiscordLogin() {
  const statusEl = document.getElementById("status");

  if (!DISCORD_CONFIG.clientId || DISCORD_CONFIG.clientId === "YOUR_CLIENT_ID_HERE") {
    statusEl.textContent = "No Client ID set yet — open config.js and add one.";
    statusEl.classList.add("err");
    return;
  }

  const params = new URLSearchParams({
    client_id: DISCORD_CONFIG.clientId,
    redirect_uri: DISCORD_CONFIG.redirectUri,
    response_type: "token",
    scope: DISCORD_CONFIG.scope,
  });

  window.location.href = `https://discord.com/oauth2/authorize?${params.toString()}`;
}

async function handleRedirectIfPresent() {
  const statusEl = document.getElementById("status");
  if (!window.location.hash.includes("access_token")) return;

  const hashParams = new URLSearchParams(window.location.hash.slice(1));
  const accessToken = hashParams.get("access_token");
  if (!accessToken) return;

  statusEl.textContent = "Verifying your account...";

  try {
    const res = await fetch("https://discord.com/api/users/@me", {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    if (!res.ok) throw new Error("Discord rejected the token");
    const user = await res.json();

    const avatarUrl = user.avatar
      ? `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png?size=128`
      : `https://cdn.discordapp.com/embed/avatars/${Number(user.discriminator || 0) % 5}.png`;

    const displayName = user.global_name || user.username;

    const dashParams = new URLSearchParams({
      username: displayName,
      handle: user.username,
      avatar: avatarUrl,
      id: user.id,
    });

    window.location.href = `dashboard.html?${dashParams.toString()}`;
  } catch (err) {
    statusEl.textContent = "Something went wrong verifying your login.";
    statusEl.classList.add("err");
  }
}

document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("discord-icon-slot").innerHTML = discordIcon();
  document.getElementById("login-btn").addEventListener("click", startDiscordLogin);
  handleRedirectIfPresent();
});
