...
// ── GitHub Release URL for template.exe ──────────────────────────────────────
//  template.exe is too large to commit (>25 MB GitHub limit).
//  It lives as a Release asset instead. This function derives the stable
//  "latest release download" URL from the current page's hostname so you
//  never have to hard-code owner/repo anywhere in the dashboard.
//
//  GitHub Pages URL pattern:  https://<owner>.github.io/<repo>/...
//  Release asset URL pattern: https://github.com/<owner>/<repo>/releases/latest/download/template.exe
//
//  Falls back to a relative "./template.exe" if the page isn't on GitHub Pages
//  (e.g. running locally via server.py) so local testing still works.
function getTemplateURL() {
  const host = window.location.hostname; // e.g. "yourname.github.io"
  const parts = host.split('.');
  if (parts.length >= 3 && parts[1] === 'github' && parts[2] === 'io') {
    const owner = parts[0];
    // pathname is "/<repo>/dashboard.html" — grab the first segment
    const repo  = window.location.pathname.split('/').filter(Boolean)[0] || '';
    if (repo) {
      return `https://github.com/${owner}/${repo}/releases/latest/download/template.exe`;
    }
  }
  ...
  genBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(
        ...
        "Push Agent.cs to GitHub — Actions will build and upload it to Releases automatically."
      );
...
async function updateBuilderStatus() {
  ...
  try {
    const r = await fetch(getTemplateURL(), { method: "HEAD" });
    if (r.ok) {
      chip.textContent = "● Ready — Firebase configured, Release asset found";
      chip.style.color = "#4dff88";
    } else {
      chip.textContent = "● Firebase OK — Release asset missing (push Agent.cs to trigger build)";
